...
Moving to the Pearson correlations, two characteristis (`resff3_6_1` and `resff3_12_1`), have low correlations, but these low correlations are solely due to outliers. The 1st and 99th percentiles of the characteristics are effectively identical between SAS and Python. Beside these two charcteristics, `sale_emp` and `bidaskhl_21d` has a Pearson correlation of 0.977 and 0.991, respectively, and all other characteristics have a Pearson correlation above 0.997. We provide a wide range of summary statistics to compare characteristics in SAS/R and Python in [`sas_vs_py_summ_stats.parquet`](data/sas_vs_py_summ_stats.parquet).^[In the new Python code, the 'div' and 'eqnpo' characteristics in the `market_chars_monthly` subroutine are coerced to 0 whenever their magnitude falls below 1e-5, in order to eliminate arithmetic noise. When comparising characteristics values, we have likewise set 'div' and 'eqnpo' to 0 in the SAS results whenever their magnitued is smaller than 1e-5. In the portfolio return comparisons, however, we use the original SAS/R output, that is, before applying the zero-floor adjustments to `div_*` and `eqnpo_*`.]
...
We now repeat the comparison for the World ex-US region. Figure @fig-comparison4 shows that the return correlations are high, but lower than in the U.S. The differences mainly arise in earlier years where only few stocks have a non-missing characteristic, and where small changes in which stocks are longed and shorted can have a big difference on the factor return. Figure @fig-comparison5, however, show that the mean and standard deviation of the factors across the two datasets align closely.   
```{python}
...
Finally, we examine correlations at the factor–country level. Figure @fig-comparison6 summarizes these comparisons by plotting the inverse cumulative distribution of the correlations for each weighting scheme (equal-weigths, value-weigths, and capped-value-weights). The figure shows that the vast majority of factors across countries and weighting scheme achieves a correlation close to 1.
